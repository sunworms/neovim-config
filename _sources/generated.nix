# This file was generated by nvfetcher, please do not modify it manually.
{
  fetchgit,
  fetchurl,
  fetchFromGitHub,
  dockerTools,
}:
{
  mnw = {
    pname = "mnw";
    version = "0151d9fa87992cbeb67c86616fb9170dcec830b6";
    src = fetchTarball {
      url = "https://github.com/Gerg-L/mnw/archive/0151d9fa87992cbeb67c86616fb9170dcec830b6.tar.gz";
      sha256 = "sha256-HRn4O1X2ShYtbq5Egji72/aemyc2cIQicjRZD9hBqM8=";
    };
    date = "2026-07-15";
  };
  nixpkgs = {
    pname = "nixpkgs";
    version = "26.11pre1066106.3ed67ec0a4d3";
    src = fetchTarball {
      url = "https://releases.nixos.org/nixos/unstable/nixos-26.11pre1066106.3ed67ec0a4d3/nixexprs.tar.zst";
      sha256 = "sha256-7VBL4mi5M72FJtZOQt7SbCGWpGbsuH1ga6Zeu8bjtWk=";
    };
  };
}
